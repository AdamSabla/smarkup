...
import type { FileEntry, UpdateStatus, SmarkupApi } from '../../../preload'

type MockFile = { path: string; name: string; content: string }
...
const listEntries = (): FileEntry[] =>
  files
    .map<FileEntry>((f) => ({ name: f.name, path: f.path, isDirectory: false }))
...
const mockApi: SmarkupApi = {
  ...
  createFile: async (parentDir, name) => {
    ...
    files.push({ path, name: safeName, content: '' })
    return path
  },
...
export const installBrowserMockApi = (): void => {
  ...
  // Mark it so TitleBar etc. can show a "browser preview" affordance later.
