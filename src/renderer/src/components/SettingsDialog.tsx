...
const SettingsDialog = (): React.JSX.Element => {
  ...
  return (
    <Dialog open={settingsOpen} onOpenChange={(open) => !open && closeSettings()}>
      {/* The settings list outgrew the viewport, so the dialog is capped at
          85vh and only the list scrolls — the title and close button stay put.
          Flex column (not the default grid) so `flex-1 min-h-0` gives the
          scroll area a definite height; without one its viewport just grows
          to fit the content and nothing scrolls. */}
      <DialogContent className="flex max-h-[85vh] flex-col overflow-hidden sm:max-w-md">
        ...
        {/* Negative margins let the scrollbar sit at the dialog's edge while
            the padding inside keeps the content aligned with the header. */}
        <div className="-mx-6 -mb-6 min-h-0 flex-1 overflow-y-auto">
          <div className="space-y-5 px-6 pt-2 pb-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                How files open when they have no saved preference yet. Switching the editor for a
                file remembers that choice for the file.
              </p>
              <div className="flex gap-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Show an H1–H4 tag in the margin next to each heading, so the hierarchy is
